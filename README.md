...
### 조회 규칙 — `CLAUDE.md` 에 추가

기록은 훅이 자동으로 하지만, **읽는 것은 규칙으로 정한다.** 세션 시작 시 무조건 주입하면
단발 질문 세션까지 수천 토큰을 물기 때문이다(실측 월 $4 대). 읽을지 여부는 모델이 요청을 보고 판단한다.

```markdown
## 진행 중 작업 조회
사용자가 이어서 할 일·진행 상황을 묻거나 기존 주제를 이어서 작업하려 하면
`<vault>/INDEX.md` 를 읽는다. 그 외의 요청이면 읽지 않는다.
정본은 `<vault>/topics/<slug>.md`, 설계 문서 경로는 그 파일 frontmatter 의 `plan:` 에 있다.
```
